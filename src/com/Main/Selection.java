...
import com.ui.SelectionWithForm;
...
import static com.ui.frames.UserFrame.ADDRESS;
import static com.ui.frames.UserFrame.BIRTH;
import static com.ui.frames.UserFrame.GENDER;
import static com.ui.frames.UserFrame.GRADE;
import static com.ui.frames.UserFrame.MEDIUM;
import static com.ui.frames.UserFrame.NAME;
import static com.ui.frames.UserFrame.SUBGRADE;
...
public class Selection extends javax.swing.JInternalFrame {
    ...
    public Selection() {
        ...
        CommonUi.setLimit(age1, Student.MINAGE, Student.MAXAGE);
        CommonUi.setLimit(age2, Student.MINAGE, Student.MAXAGE);
        try {
            tableLoad();
        } catch (Exception ex) {
    ...
    public String[] getSelectedIds() {
        return CommonUi.getSelectedArray(0, Table);
    }
...
//        System.out.println(sql);
        CommonUi.tableload(sql, Table, CONN);
    }
    ...
    public String[] getColumns() {
        ...
        for (int i = 0; i < cols.length; i++) {
            if (cols[i]) {
                if (COLUMNNAMES[i].equals(UserFrame.BIRTH)) {
                    col.add("strftime('%Y-%m-%d', \"Date of birth\" / 1000, 'unixepoch') AS 'Date of birth'");
                } else {
                    col.add(getCoveredTable(COLUMNNAMES[i]));
                }
            }
        }
    ...
    public String getQuery() {
        String query = "";
        if (!n.getText().equals("")) {
            query = query.concat(" " + getCoveredTable(NAME) + " LIKE " + getCoveredLike(n.getText()) + " " + or);
        }
//        if (!CommonUi.gsi(ageSel).equals(all)) {
//            int f = (int) age1.getValue();
//            int s = (int) age2.getValue();
//            switch (CommonUi.gsi(ageSel)) {
//                case "From-To":
...
//            }
//        }
        if (!CommonUi.gsi(gradeSel).equals(all)) {
            int f = Integer.parseInt(CommonUi.gsi(jComboBox3));
            int s = Integer.parseInt(CommonUi.gsi(jComboBox4));
            switch (CommonUi.gsi(gradeSel)) {
                case "From-To":
                    query = query.concat(" " + getCoveredTable(GRADE) + " BETWEEN " + getCovered(f + "") + " AND " + getCovered((s + 1) + "") + or);
                    break;
                case "And":
                    query = query.concat(" " + getCoveredTable(GRADE) + "=" + getCovered(f + "") + (CommonUi.gsi(subGrade).equals(all) ? "" : " AND " + getCoveredTable(SUBGRADE) + "=" + getCovered(CommonUi.gsi(subGrade)) + " ") + " " + or + getCoveredTable(GRADE) + "=" + getCovered(s + "") + (CommonUi.gsi(subGrade).equals(all) ? "" : " AND " + getCoveredTable(SUBGRADE) + "=" + getCovered(CommonUi.gsi(subGrade)) + " "));
                    break;
                case "Equals":
                    query = query.concat(" " + getCoveredTable(GRADE) + "=" + getCovered(s + "") + (CommonUi.gsi(subGrade).equals(all) ? "" : " AND " + getCoveredTable(SUBGRADE) + "=" + getCovered(CommonUi.gsi(subGrade)) + " ") + " " + or);
                    break;
            }
        }
        if (!mediumAll.isSelected()) {
            query = query.concat((sinhala.isSelected() ? " " + getCoveredTable(MEDIUM) + (noMedium > 1 ? " LIKE " : " = ") + getCovered("Sinhala") + " " + or : "") + (english.isSelected() ? " " + getCoveredTable(MEDIUM) + (noMedium > 1 ? " LIKE " : " = ") + getCovered("English") + " " + or : "") + (tamil.isSelected() ? " " + getCoveredTable(MEDIUM) + (noMedium > 1 ? " LIKE " : " = ") + getCovered("Tamil") + " " + or : ""));
        }
        if (!bod1.getDate().equals(bod1.getMinSelectableDate()) && !bod2.getDate().equals(bod2.getMaxSelectableDate())) {
            query = query.concat(" " + getCoveredTable(BIRTH) + " BETWEEN " + getCoveredTable(CommonUi.getDate(bod1)) + " AND " + getCoveredTable(CommonUi.getDate(bod2)) + " " + or);
        }
        if (!gendAll.isSelected()) {
            query = query.concat((male.isSelected() ? " " + getCoveredTable(GENDER) + (noGender > 1 ? " LIKE " : " = ") + getCovered("Male") + " " + or : "") + (female.isSelected() ? " " + getCoveredTable(GENDER) + (noGender > 1 ? " LIKE " : " = ") + getCovered("Female") + " " + or : "") + (other.isSelected() ? " " + getCoveredTable(GENDER) + (noGender > 1 ? " LIKE " : " = ") + getCovered("Other") + " " + or : ""));
        }
        if (!jTextField1.getText().equals("")) {
            query = query.concat(" " + getCoveredTable(ADDRESS) + " LIKE " + getCoveredLike(jTextField1.getText()) + " ");
        }
...
//            rowSorter.setRowFilter(RowFilter.regexFilter("(?i)" + text, 0));
    ...
    public void gradeSelActionPerformed(java.awt.event.ActionEvent evt) {
        String selection = CommonUi.gsi(gradeSel);
        String s1 = "From";
        String s2 = "To";
        boolean sg = true;
        boolean g = true;
    ...
    public void ageSelActionPerformed(java.awt.event.ActionEvent evt) {
        String selection = CommonUi.gsi(ageSel);
        String s1 = "From";
        String s2 = "To";
        switch (selection) {
            case "*All":
    ...
    public void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {
        CommonUi.genarateCenter(jDialog1, 530, 395);
